...
from imbalance_data.imbalance_cifar import IMBALANCECIFAR10
...
from models.DVM_base import CustomCIFAR10
...
def validate(val_loader, model, criterion, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    with torch.no_grad():
        ...
        # Compute class accuracy for head, middle, and tail
        sorted_indices = np.argsort(train_cls_num_list)[::-1]  # Sort classes by count in descending order
        num_classes = len(train_cls_num_list)
        head_classes = sorted_indices[:num_classes // 3] 
        middle_classes = sorted_indices[num_classes // 3:2 * num_classes // 3] 
        tail_classes = sorted_indices[2 * num_classes // 3:] 

        head_acc = np.mean(cls_acc[head_classes]) * 100
        middle_acc = np.mean(cls_acc[middle_classes]) * 100
        tail_acc = np.mean(cls_acc[tail_classes]) * 100

        print("Head Accuracy: {:.2f}%, Middle Accuracy: {:.2f}%, Tail Accuracy: {:.2f}%".format(
            head_acc, middle_acc, tail_acc))
