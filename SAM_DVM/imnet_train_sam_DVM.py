...
from utils import *
from datasets.imagenet_base import ImageNet_LT
...
def main():
    ...
    if args.rho_steps is None:
        args.rho_steps = []
    args.store_name = '_'.join(
        [args.dataset, args.arch, args.loss_type, args.train_rule, args.imb_type, str(args.imb_factor), 'sam',
         'rho_step', '-'.join(map(str, args.rho_steps)), args.exp_str])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.loss_type == 'CE':
            ...
            criterion = LDAMLoss(cls_num_list=cls_num_list, max_m=args.margin, s=30, weight=per_cls_weights).cuda(
                args.gpu)
